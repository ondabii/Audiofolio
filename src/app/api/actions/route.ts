  ...
  try {
    ...
    switch (action) {
      ...
      case "verifyAdminPin":
        ...
        const pinRes = await executeActionQuery("SELECT value FROM admin_config WHERE key = 'admin_pin' OR key = 'admin_key' ORDER BY rowid DESC");
        const systemPin = pinRes.results?.[0]?.value || '161017';
      ...
      case "updateAdminPin":
        ...
        await executeActionQuery(
          "INSERT OR REPLACE INTO admin_config (key, value) VALUES ('admin_key', ?)",
          [payload.pin]
        );
        break;
